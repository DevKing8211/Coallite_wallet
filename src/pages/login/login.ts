...
export class LoginPage {
  ...
  setNode() {
    ...
    this.accountData.setNode("mainnet/").then(() => {
      this.accountData.checkNode().subscribe(
        ...
        (err) => {
          let nodeError = this.accountData.getNodeFromMemory();
          this.error = `Node (${nodeError}) not online`;
          this.loading = false;
        });
    });
  }
  ...
  onLogin(account: string, type: string = "Account") {
    ...
    this.shared.getConstantsHttp().subscribe((shared) => {
      ...
      if (type == "Account" && (account.substring(0, 4) != "UST-" && account.substring(0, 5) != "COAL-" && account.substring(0, 4) != "SGT-" && account.substring(0, 4) != "MYR-" && account.substring(0, 6) != "ASEAN-")) {
      this.accountData.getAlias('ust', account)
        .subscribe(
          alias => {
