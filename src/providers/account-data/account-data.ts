...
export class AccountDataProvider {
  ...
  FINGER_SECRET: string = "mrV13wa!!eTlC0a1";
  // MAINNET_NODES = ['https://capp.coalculus.com/'];
  MAINNET_NODES = ['https://testcoal.coalculus.com/'];
  // TESTNET_NODES = ['https://capp.coalculus.com/'];
  TESTNET_NODES = ['https://testcoal.coalculus.com/'];

  
  SUCCESS_URL = "http://paycoal.coalculus.com/payment_success.php";
  ...
  init(): Promise<void> {
    ...
    return this.secureStorage.create('coal_lite_accounts')
  ...
  login(password: string, loginType: string): void {
    if (loginType == "Account") {
    	if (password.substring(0, 4) == "NXT-") {
	      password = password.replace("NXT-","COAL-");
	    } else if (password.substring(0, 6) == "ARDOR-") {
        password = password.replace("ARDOR-","COAL-");
      }
  ...
  getSavedGuest(): Promise<string> {
    return this.storage.get(`CoalGuestAccount`).then((value) => {
        return value;
    });
  }
  ...
  setNode(node: string): Promise<void> {
    ...
    // }
     ...
     //this.NODE_URL = "https://capp.coalculus.com/";
    this.NODE_URL = "https://testcoal.coalculus.com/";
  ...
  verifyAndSignTransaction(unsignedTransactionBytes: string, password: string, request: string, data: object): string  {
    console.log('star_verifyAndSignTransaction() is called: password:' + password);
    if (ardorjs.verifyTransactionBytes(unsignedTransactionBytes, request, data, this.PUBLIC_KEY)) {
      ...
      signPassword = signPassword.replace("ARDOR","COAL");
      console.log('star_signPassword:' + signPassword);
